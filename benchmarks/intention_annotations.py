...
def generate_intention_annotations(ann_dir, output_dir, categories):
    last_id = 1
    total_seq = 0
    ...
    for vn, path in vids_ann.items():
        ...
        objects_predictions = {pd['object_id']: {'class': pd['class'], 
                                                   'frames': pd['frames'], 
                                                   'bbox': pd['bbox'], 
                                                   'intention':pd['intention']} for pd in prediction_labels.values()}
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Creating intention annotations')
    ...
    output_dir = os.path.join(args.dataset_dir, "annotations/intention")
    os.makedirs(output_dir, exist_ok=True)
    
    categories = dict(list(AGENT_LABELS.items())[:9])        
    generate_intention_annotations(ann_dir, output_dir, categories)